    ...
    const { name, email, password } = req.body;  
        ...
        const emailData = {
            ...
            html: `
                ...
                <hr />
                <p>This email may contain senstive information</p>
...
exports.accountActivation = (req, res) => {
    ...
    if (token) {
        jwt.verify(token, process.env.JWT_ACCOUNT_ACTIVATION, (err, decoded) => {
            if (err) {
                console.log('JWT CERIFY IN ACCOUNT ACTIVATION ERROR', err);
            ...
            user.save((err, user) => {
                ...
                return res.json({
                    message: 'Signup successful! Please proceed to signin.'
                });
            });
        });
    } else {
    ...
    return (
        fetch(url, {
           method: 'GET' 
            ...
            .catch(err => {
                res.json({
                    error: 'Facebook login failed. Please try later.'
                });
            })
    )
};